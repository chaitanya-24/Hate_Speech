...
class DataTransformation:
    ...
    def raw_data_cleaning(self):
        try:
            ...
            # Lets replace the value of 2 to 0
            raw_data[self.data_transformation_config.CLASS].replace({2:0}, inplace=True)

            # Lets change the name of the 'class' to label
            raw_data = raw_data.rename(columns={self.data_transformation_config.CLASS: self.data_transformation_config.LABEL}, inplace=True)
    ...
    def concat_dataframe(self):
        try:
            ...
            # Lets concatinate both the data into a single data frame.
            frame = [self.raw_data_cleaning(), self.imbalance_data_cleaning()]
            df = pd.concat(frame)
            print(df.head())
            logging.info(f"Exited the concat_dataframe function and returned the df {df}")
            return df
